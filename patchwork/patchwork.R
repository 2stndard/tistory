...
data %>%
  ...
  geom_density(aes(y=0.25 * ..count..), alpha = 0.3) + 
  labs(y = '밀도분포') + 
...
data %>%
  ...
  scale_y_discrete(limits=rev) + coord_cartesian(xlim = c(5, 26)) -> p_시도_초등학교
...
data %>%
  filter(학교급 %in% c('초등학교')) %>%
  mutate(학급당학생수 = 학생수_계_계 / 학급수_계) %>%
  ggplot(aes(x = 학급당학생수, y = fct_relevel(시도, '서울', '부산', '대구', '인천', '광주', '대전', '울산', '세종', '경기', '강원', '충북', '충남', '전북', '전남', '경북', '경남', '제주'), group = 시도)) +
  geom_density_ridges()+ 
  scale_y_discrete(limits=rev) +
  labs(y = '시도')
             
             
             fill = fct_relevel(학교급, '계', '유치원', '초등학교', '중학교', '고등학교', '(일반고)', '(자율고)', '(특목고)', '(특성화고)', '고등공민학교', '고등기술학교', '각종학교'))) + 
