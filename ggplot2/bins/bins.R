...
df.전처리 |>
  ...
  ggplot(aes(x = 입학자_전체_계)) +
  stat_bin(aes(y = ..count..), geom = 'density') + 
  stat_bin(aes(y = ..count.., label = ..count..), geom = 'text', vjust = -0.2)
...
df.전처리 |>
  filter(입학자_전체_계 != 0) |>
  ggplot(aes(x = 입학자_전체_계)) +
  geom_histogram(aes(y = ..ncount..), bins = 10, center = FALSE) +
  stat_bin(aes(y=..ncount.., label=..ncount..), bins = 10, geom="text", vjust=-.5) +
  labs(x = '재적학생수', y = '학과수') 
