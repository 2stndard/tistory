...
이제 전체적인 표의 형태가 대략 갖춰진 듯 하다. 마지막으로 각 행 그룹의 합계, 평균 행을 넣고 마지막에 전체 합계, 평균 행을 넣어주겠다. 각 행 그룹의 요약 행을 넣는 함수는 `summary_rows()`이고 전체 요약 행을 넣는 함수는 `grand_summary_rows()`이다. 
